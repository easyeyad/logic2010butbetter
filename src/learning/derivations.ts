...
import { checkDerivation } from '../proof';
...
function toExercise(p: Problem): DerivationExercise {
...
export const DERIVATION_EXERCISES: DerivationExercise[] = PROBLEMS.map(toExercise);
...
export function derivationSolutionDraft(ex: DerivationExercise): DerivationDraft {
  return { ...parseProofScript(ex.solution, ex.goal, `${ex.id}-s`), allowDerivedRules: ex.allowDerivedRules };
}
...
export function derivationHints(ex: DerivationExercise): string[] {
  ...
  const outline = parseProofScript(ex.solution)
    .lines.map((l, i) => ({ l, i }))
