...
const PREDICATE_ITEMS: Item[] = [
  ...
  { id: 'pterm-19', difficulty: 3, concept: 'identity', format: 'fill-in', prompt: 'The principle "if a = b, then whatever is true of a is true of b" is ___\'s law (the indiscernibility of identicals).', accepted: ['Leibniz', "Leibniz's", 'Leibniz law'], explanation: "Leibniz's law (LL): identicals share all their properties, so you may substitute one name for the other." },
  { id: 'pterm-20', difficulty: 3, concept: 'identity', format: 'true-false', prompt: 'True or false: a = a is true in every interpretation.', truth: true, options: ['Identity is reflexive: every object is identical to itself.', 'Only if a names something with a property.', 'It depends on the domain size.'], correctOption: 0, explanation: 'Reflexivity — the rule Id lets you write t = t at any point.' },
  { id: 'pterm-21', difficulty: 3, concept: 'identity', format: 'true-false', prompt: 'True or false: from a = b you may infer b = a.', truth: true, options: ['Identity is symmetric (the rule SM).', 'Only if b = a is a premise.', 'Only by EG.'], correctOption: 0, explanation: 'Symmetry: if a is b, then b is a.' },
  { id: 'pterm-22', difficulty: 4, concept: 'identity', format: 'true-false', prompt: 'True or false: Fa ∧ ¬Fb is consistent with a = b.', truth: false, options: ["By Leibniz's law, if a = b then Fa and Fb have the same truth value.", 'Different names always name different things.', 'Identity only concerns variables.'], correctOption: 0, explanation: 'If a and b are the same object, it cannot both have and lack F.' },
  { id: 'pterm-23', difficulty: 4, concept: 'identity', format: 'fill-in', prompt: '"Exactly one" combines "at least one" with "at ___ one".', accepted: ['most'], explanation: 'Exactly one F: ∃x(Fx ∧ ∀y(Fy → y = x)).' },
  { id: 'pterm-24', difficulty: 4, concept: 'identity', format: 'true-false', prompt: 'True or false: ∃x∃y(Fx ∧ Fy) says there are at least two F\'s.', truth: false, options: ['x and y may be the same object — you need ¬(x = y) as well.', 'It says there are exactly two.', 'It is not well-formed.'], correctOption: 0, explanation: 'At least two: ∃x∃y((Fx ∧ Fy) ∧ ¬(x = y)).' },
];
